...
public class DeviceIssueInfoREST {
    private static final Logger LOGGER = Logger.getLogger(DeviceIssueInfoREST.class);
    ...
    public JSONObject deviceIssueReportService(@QueryParam("beginDate") String beginDate,
            @QueryParam("endDate") String endDate, @QueryParam("deviceId") String deviceId) {
        return deviceIssueInfoService.getDeviceTimeLineReport(beginDate, endDate, deviceId,
                Constants.REPORT_DEVIVE_TIMELINE);
    }

    @GET
    @Path("/userTimeLineReport")
    @Produces(MediaType.APPLICATION_JSON)
    public JSONObject userIssueReportService(@QueryParam("beginDate") String beginDate,
            @QueryParam("endDate") String endDate, @QueryParam("userId") String userId) {
        return deviceIssueInfoService.getDeviceTimeLineReport(beginDate, endDate, userId,
                Constants.REPORT_USER_TIMELINE);
    }
    ...
    @Produces(MediaType.APPLICATION_JSON)
    public Response getDeviceIssueTimeLineTrendReport(@QueryParam("beginDate") String beginDate,
            @QueryParam("endDate") String endDate, @QueryParam("reportType") String reportType) {
        Response response = null;
        DeviceIssueTrendLineDto deviceIssueTimeLineTrendReport = null;
        try {
            deviceIssueTimeLineTrendReport = deviceIssueInfoService.getDeviceIssueTimeLineTrendReport(beginDate,
                    endDate, reportType);

            if (deviceIssueTimeLineTrendReport == null) {
                response = Response.ok(deviceIssueTimeLineTrendReport, MediaType.APPLICATION_JSON).entity("No data")
                        .build();
            } else {
    ...
    public Response getDeviceIssueStatusForDevice(@QueryParam("deviceId") String deviceId) {
        DeviceIssueStatusDto deviceIssueStatusDto = deviceIssueInfoService.getDeviceIssueStatusForDevice(deviceId);
        Response response;
        if (deviceIssueStatusDto != null) {
            response = Response.ok(deviceIssueStatusDto, MediaType.APPLICATION_JSON).entity(deviceIssueStatusDto)
                    .build();
        } else {
            return Response.ok(new DeviceIssueStatusDto(), MediaType.APPLICATION_JSON).status(400).build();
        }
        return response;
    }
